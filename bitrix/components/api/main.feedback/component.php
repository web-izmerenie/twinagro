...
else
{
	...
	if($arParams['CUSTOM_FIELDS'])
	{
		foreach($arParams['CUSTOM_FIELDS'] as $k1 => $v1)
		{
			...
			if(in_array('text',$arExplodeFields) || in_array('hidden',$arExplodeFields) || in_array('coupon',$arExplodeFields) || in_array('date',$arExplodeFields) || in_array('textarea',$arExplodeFields))
            {
                if($arExplodeFields)
                {
